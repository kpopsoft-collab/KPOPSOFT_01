...
>
> **2026-08-06 통과** — 2026-08-05에 위젯을 고쳤으므로(`DetailSource` 3지 → 2지,
> `htmlIntent` 상수화) 다시 확인했다. 공개 페이지에서 자료가 실제로 열리는
> 것까지 봤다. **위젯의 라디오·intent 분기를 손대면 이 테스트를 또 해야 한다.**
>
> 손실이 되돌릴 수 없어서 이 한 줄이 중요하다 — 컬럼이 비는 것으로 끝나지 않고
> §3-4의 `discardBundleFolder`가 **Storage 폴더까지 지운다.** zip 번들은 Storage가
> 원본이라 백업이 없다(옛 `detail_html`은 §3-1의 동반 테이블에 원본이 남는다).
