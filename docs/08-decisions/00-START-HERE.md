...
- **CSP 강제 전환** — 정적 페이지에 nonce가 안 박혀 `/admin/login`이 막힌다.
  [../07-dev/14](../07-dev/14-CSP-정책과-적용.md) §5
- 어드민 화면 사람 검증 몇 건 — [../07-dev/05](../07-dev/05-남은결정과-작업.md)

## 새 기능을 시작할 때

폴더를 하나 만들고 `00-START-HERE.md`부터 쓴다. **요구사항 → 현황분석 → 결정 →
구현계획 → 검증체크리스트 → 구현결과**가 여기 일곱 건의 공통 골격이다.
끝나면 "지금 어떻게 동작하나"를 앞 번호 폴더의 상시 문서로 올리고, 이 폴더에는
**왜 그렇게 정했나**만 남긴다.
