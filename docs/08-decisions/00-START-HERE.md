...
| 남은 것 | 왜 |
|---|---|
| `/education` 문의 폼 실측 | 홈 폼과 같은 Server Action이라 통과가 유력하지만 **직접 눌러 보지 않았다.** 유추로 체크하지 않는다 |
| 수신 라우팅 규칙(D6) | `inquiry@kpopsoft.com` → Gmail 전달. 토큰에 `Zone:Read`가 없어 API로 확인 못 했다 — 대시보드에서 봐야 한다 |
| 매퍼 테스트 재작성 | 이식한 테스트가 가짜 응답으로 돈다. **실제 응답 픽스처**로 다시 쓴다 |
