...
**발송은 운영에서 확인됐다**(2026-08-11) — 홈 Contact와 `/education` 문의 폼
둘 다 실측했다. WP 258이 신고한 증상은 해소됐다. 아래는 그 뒤에 남은 것이다.

| 남은 것 | 왜 |
|---|---|
| 수신 라우팅 규칙(D6) | `inquiry@kpopsoft.com` → Gmail 전달. 발송과 별개 축이고, 토큰에 `Zone:Read`가 없어 API로 확인 못 했다 — 대시보드에서 봐야 한다. **KPO-23은 이게 닫혀야 끝난다** |
