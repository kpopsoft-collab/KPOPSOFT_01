...
`CLAUDE.md`의 "문서 하나는 200줄을 넘지 않는다"를 아래 다섯이 어긴다.
...
> 9의 두 파일은 2026-08-11에 새로 쓰면서 넘겼다 — 조사가 저장소 안(코드·env)과
> 밖(DNS·Cloudflare 계정·실제 API 응답) 양쪽에 걸쳐서 한 호흡으로 붙어 있다.
> 쪼개면 "메일이 왜 안 갔나"의 인과가 두 파일로 끊긴다. **다음에 쓰는 결정
> 기록은 규칙을 지킨다.**
...
| 9 | [09-inquiry-email-notification](09-inquiry-email-notification/00-START-HERE.md) | **버그** — 문의 알림 메일이 두 달간 안 나갔다. Resend → **Cloudflare** 교체 | 2026-08-11 → [06-구현결과](09-inquiry-email-notification/06-구현결과.md) · [../06-admin/09](../06-admin/09-문의-알림-메일.md) |
...
| **`06-admin/03-…:61`의 "예: Resend"** | `kpopsoft.com`의 NS·MX·SPF가 이미 전부 Cloudflare였다. Resend로 보내면 SPF에서 떨어진다. KPO-23 승인 설계가 이미 Cloudflare로 정해 뒀는데 저장소 문서만 낡아 있었다 | [9의 03](09-inquiry-email-notification/03-원인과-결정.md) D2 |
| 9의 **이식한 응답 매퍼** | 이식 원본이 세 배열 빈 응답을 `provider_error`로 판정했는데, **그게 정상 발송의 응답 모양**이었다. 그대로 뒀으면 성공할 때마다 거짓 실패 로그가 쌓였다 | [9의 03](09-inquiry-email-notification/03-원인과-결정.md) D7 |

> **교훈 하나만 고르면** — "파일이 올라갔다"를 "화면이 나온다"의 증거로 쓰지 않는다.
> 링크를 만들었으면 그 링크를 연다.
>
> **9가 같은 교훈을 다른 얼굴로 반복했다** — 폼의 "접수되었습니다"를
> "메일이 갔다"의 증거로 쓴 것이 두 달을 잡아먹었다. 그리고
> **테스트가 초록인 것도 증거가 아니었다** — 매퍼 테스트는 가짜 응답으로만
> 돌아서 실제 응답 모양을 한 번도 본 적이 없었다.

## 남은 일 — **9에 몇 건 있다**

2026-08-11 기준. 상세는 [9의 06-구현결과](09-inquiry-email-notification/06-구현결과.md).

**발송은 운영에서 확인됐다**(2026-08-11) — WP 258이 신고한 증상은 해소됐다.
아래는 그 뒤에 남은 것이다.

| 남은 것 | 왜 |
|---|---|
| `/education` 문의 폼 실측 | 홈 폼과 같은 Server Action이라 통과가 유력하지만 **그 폼으로 직접 눌러 본 기록이 없다.** 유추로 체크하지 않는다 |
| 수신 라우팅 규칙(D6) | `inquiry@kpopsoft.com` → Gmail 전달. 발송과 별개 축이고, 토큰에 `Zone:Read`가 없어 API로 확인 못 했다 — 대시보드에서 봐야 한다 |
| 매퍼 테스트 재작성 | 이식한 테스트가 가짜 응답으로 돈다. **틀린 매퍼를 통과시킨다.** 실제 응답 픽스처로 다시 쓴다 |
| `resend` 의존성 제거 | 코드 참조는 없지만 `package.json`에 남아 있다 |
| Cloudflare 토큰 재발급 | 채팅으로 평문 전달됐다 |
