...
- **CSP 강제 전환** — 정적 페이지에 nonce가 안 박혀 `/admin/login`이 막힌다.
  확인 체크리스트 8개는 2026-08-06에 통과했고, **남은 것은 이 문제 하나다.**
  [../07-dev/14](../07-dev/14-CSP-정책과-적용.md) §5
- 어드민 회귀 1건 — *번들 있는 과정을 이름만 고쳐 저장해도 자료가 남는가.*
  [../07-dev/05](../07-dev/05-남은결정과-작업.md)

`ai-tools` 상세 자료 404는 2026-08-06 재업로드로 해소됐다.
