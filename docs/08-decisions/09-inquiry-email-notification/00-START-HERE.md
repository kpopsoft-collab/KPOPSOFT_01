...
**✅ 구현 완료 — 로컬에서 문의 폼 제출 시 실제로 메일이 나간다.**
결과와 계획 대비 차이는 [06-구현결과.md](06-구현결과.md).

**⚠️ 운영은 아직 안 나간다** — Vercel env 4개를 넣고 재배포해야 한다.

**남은 것**

1. **Vercel 운영 env + 재배포** ← 이걸 해야 실제로 고쳐진다
2. `inquiry@kpopsoft.com` 라우팅 규칙(D6) — 대시보드에서 눈으로
3. `cloudflare` SDK 대신 `fetch`를 쓴 것을 받아들일지 ([06](06-구현결과.md) §1)
4. main에서 `resend` 의존성을 뺄 것인가
5. 문서 갱신 5곳 · OpenProject 정리 · **토큰 재발급**
