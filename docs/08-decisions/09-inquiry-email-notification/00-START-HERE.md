...
**✅ 발송은 끝났다 — 운영에서 폼 두 개 모두 수신 확인**(2026-08-11).
홈 Contact와 `/education` 문의 폼 둘 다 실측했다. WP 258이 신고한 증상은
해소됐다. 결과와 계획 대비 차이는 [06-구현결과.md](06-구현결과.md).

**남은 것**

1. `inquiry@kpopsoft.com` 라우팅 규칙(D6) — 대시보드에서 눈으로.
   **KPO-23은 이게 닫혀야 끝난다**(본문이 "발송 / 수신" 두 줄)
2. 매퍼 테스트를 실제 응답 픽스처로 재작성
3. `cloudflare` SDK 대신 `fetch`를 쓴 것을 받아들일지 ([06](06-구현결과.md) §1)
4. **토큰 재발급** — 채팅으로 평문 전달됐다
