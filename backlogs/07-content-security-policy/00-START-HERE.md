...
> [docs/07-dev/14-CSP-정책과-적용.md](../../docs/07-dev/14-CSP-정책과-적용.md)로 승격돼 있다.**
> 이 폴더는 이제 "왜 그렇게 정했나"의 조사·결정 기록이다.
> **선행** 백로그 06이 인라인 HTML 렌더링을 폐지하면서 이 작업이 가능해졌다 —
...
백로그 06과 `docs/07-dev/13`은 *"인라인 렌더링이 사라져 인라인 `<style>`을
...
**자료가 통째로 안 뜬다.** 백로그 06이 방금 고친 것이 다시 깨진다.
