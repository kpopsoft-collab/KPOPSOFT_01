...
Before writing code:
- Read `docs/00-START-HERE.md`, then the owning folder — `docs/02-home/` for home sections, `docs/03-education/` for education — for the section's purpose, copy, and structure. Lower-numbered docs win on conflict.
- Read `docs/04-design-system/` for the exact tokens (color hex, type scale, spacing, shape vocabulary, motion timing) that apply. Never invent colors, radii, or spacing that aren't in that doc — if something's ambiguous, pick the closest documented token rather than a new value.
...
Conventions:
- One component per section under `components/sections/`, matching the section order in `docs/02-home/00-START-HERE.md` (home) or `docs/03-education/00-START-HERE.md` (education).
...
- Build mobile layouts as intentional recompositions per docs/04-design-system/12-모바일과-접근성.md, not naive stacking of the desktop layout.
