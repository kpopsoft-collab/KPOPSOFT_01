...
export function InquiryForm() {
  ...
  return (
    <Section id={educationSectionId.inquiry} className="scroll-mt-36">
      <div className="relative overflow-hidden rounded-[2rem] bg-brand-navy px-6 py-14 md:rounded-[2.5rem] md:px-12 md:py-20">
        {/* Green Wave + Blue Circle 배경 그래픽(§13). 장식이므로 스크린리더에서
            제외하고, 글자 위로 올라오지 않게 뒤에 깔린다. */}
        <div aria-hidden className="pointer-events-none absolute inset-0">
          <Circle className="absolute -top-16 -right-16 size-64 text-brand-blue/25" />
          <Wave className="absolute -bottom-8 -left-10 w-72 text-brand-mint/30" />
        </div>
        ...
        <div className="relative grid grid-cols-1 gap-12 lg:grid-cols-12 lg:gap-14">
          <div className="lg:col-span-5">
            ...
            <p className="mt-6 max-w-md text-base leading-relaxed text-ivory/70 md:text-lg">
              대상과 목적, 현재 AI 활용 수준을 알려주시면 적합한 프로그램과
              진행 방식을 제안해드립니다.
            </p>
          </div>
                ...
                <form onSubmit={handleSubmit} noValidate className="relative">
                  ...
                  <div className="mt-7 grid grid-cols-1 gap-5 sm:grid-cols-2">
                    <label className={labelClass}>
                      <FieldLabel required>{isOrg ? "회사명" : "이름"}</FieldLabel>
                      <input
                        value={name}
