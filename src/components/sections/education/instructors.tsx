 ...
 * 전문 강사진 (수정 요청서 §10).
 *
 * 3열 그리드에서 캐러셀로 바꿨다. 요청서가 요구한 "선택된 강사를 크게 강조,
 * 다음 강사 카드를 일부 노출"은 그리드로는 표현할 수 없다 — 그리드는 모든
 * 항목을 동등하게 놓는 배치라 강조 자체가 성립하지 않는다.
 *
 * 이동은 **네이티브 가로 스크롤 + scroll-snap**으로 구현한다. 직접 transform을
 * 계산하는 캐러셀보다 이쪽이 얻는 게 많다 — 터치 드래그와 관성 스크롤,
 * 트랙패드 두 손가락 스와이프, 키보드 스크롤이 전부 브라우저 기본 동작으로
 * 따라오고, 스크린리더도 카드를 순서대로 읽는다. 화살표 버튼은 그 스크롤을
 * 한 칸씩 움직일 뿐이다.
 *
  ...
  return (
    <Section id={eduSectionId.instructors} className="scroll-mt-36 bg-ivory">
      <div className="flex flex-col gap-8 md:flex-row md:items-end md:justify-between">
        ...
        {/* 화살표는 카드가 화면에 다 들어오지 않을 때만 의미가 있다. 항목이
            적으면 눌러도 아무 일이 없어 고장난 것처럼 보이므로 숨긴다. */}
        {experts.length > 2 ? (
      ...
      <ul
        ref={scrollerRef}
        className={cn(
          "mt-14 flex snap-x snap-mandatory gap-6 overflow-x-auto pb-4 lg:mt-20",
          // 스크롤바를 숨기되 스크롤 자체는 살린다.
          "[scrollbar-width:none] [&::-webkit-scrollbar]:hidden",
          // 카드 폭을 100% 미만으로 둬서 다음 카드가 항상 일부 보인다(§10).
      ...
      >
        {experts.map((expert, index) => (
          <li
            key={expert.name}
            className="w-[78%] shrink-0 snap-start sm:w-[52%] lg:w-[38%]"
          >
            <InstructorCard expert={expert} active={index === active} />
          </li>
        ))}
      </ul>
    </Section>
